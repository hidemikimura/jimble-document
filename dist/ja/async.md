...
**触れば読みます——読む側も、書き換える側もです。**
`get` や `size` はもちろん、`put` / `remove` / `add` / `clear` /
`computeIfAbsent` / `putFirst` のような**書き換える口も**読み込みを起こします。

> [!NOTE]
> **0.6.x までは、書き換える口が読み込みを起こしていませんでした。**
> 未読み込みのまま `remove("id")` すると**空のマップから消す**ことになり、
> あとで誰かが読んだ瞬間に**消したはずの値が戻ってきました**。
> `add` した1件が、読み込みで**上書きされて消える**という形もありました。
> 落ちも警告も出ないので、気づけるのは**結果がおかしいと言われたとき**だけです。
